...
public class TimKiemService {
    ...
    public List<Map<String, Object>> findCustomersByEmailOrPhone(String email, String phone) {
        ...
        try {
            ...
            if (phone != null && !phone.isEmpty()) {
                ...
                ResultSet rs = session.execute(bs);
                for (Row row : rs) {
                    boolean exists = customers.stream().anyMatch(c -> c.get("customer_id").equals(row.getUuid("customer_id")));
                    if (!exists) {
                        customers.add(rowToMap(row, columns));
                    }
                }
    ...
    public Map<String, Object> getCustomerDetails(UUID customerId) {
    ...
    public Map<String, Object> findProductById(String productId) {
        ...
        try {
            ...
            Row row = rs.one();
            if (row != null) {
                List<String> columns = Arrays.asList("product_id", "brand", "model", "cpu", "ram", "storage", "price", "available", "created_at");
    ...
    public List<Map<String, Object>> getCustomerOrderHistory(UUID customerId, String yyyyMm) {
    ...
    public Map<String, Object> getCustomerLoyalty(UUID customerId) {
    ...
    public boolean updateCustomerLoyalty(UUID customerId, long pointsDelta, BigDecimal spentDelta) {
        try {
            ...
            Row row = rs.one();
            if (row == null) {
                return false; // No loyalty record found
            }
